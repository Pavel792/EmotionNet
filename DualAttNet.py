...
class EmotionAttentionNet(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.sa4 = DualAttention(512)
        self.sa5 = DualAttention(512)
...
class selfAttentionInChannel(nn.Module):
    def __init__(self, channels, reduction=8):
        super().__init__()

        interChannels = channels // reduction
        self.query = nn.Conv2d(channels, interChannels, kernel_size=1)
        self.key   = nn.Conv2d(channels, interChannels, kernel_size=1)
        self.value = nn.Conv2d(channels, interChannels, kernel_size=1)
        self.outConv = nn.Conv2d(interChannels, channels, kernel_size=1)
...
class selfAttentionBetweenChannels(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = self.softmax(torch.bmm(x, xT))

        out = torch.bmm(attention, x).view(batch, C, H, W) 
...
class DualAttention(nn.Module):
    def __init__(self, channels, reduction=8):
        super().__init__()
        self.pam = selfAttentionInChannel(channels, reduction)
        self.cam = selfAttentionBetweenChannels()
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc:
        ...
        torch.save(model.state_dict(), 'best_models/DualAttNet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/DualAttNet.pt'))
model.eval()
testCorrect = 0
testTotal = 0

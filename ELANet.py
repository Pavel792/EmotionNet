...
class EmotionCNN(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.att1 = ELABlock(64)
        self.att2 = ELABlock(128)
        self.att3 = ELABlock(256)
        self.att4 = ELABlock(512)
        self.att5 = ELABlock(512)
...
class ELABlock(nn.Module):
    def __init__(self, channels, kernelSize=5, numGroup=16):
        super().__init__()
        ...
        self.convH = nn.Conv1d(in_channels=channels, out_channels=channels, padding="same", kernel_size=kernelSize, groups=channels//8)
        self.convW = nn.Conv1d(in_channels=channels, out_channels=channels, padding="same", kernel_size=kernelSize, groups=channels//8)

        self.gnH = nn.GroupNorm(numGroup, channels)
        self.gnW = nn.GroupNorm(numGroup, channels)
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc or _e == 0:
        ...
        torch.save(model.state_dict(), 'best_models/ELANet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/ELANet.pt'))
model.eval()
testCorrect = 0
testTotal = 0
...
# Max Acc without overfitting 0.7832

...
class EmotionCNN(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.att4 = TripletAttention()
        self.att5 = TripletAttention()
...
class ZPoolAttention(nn.Module):
    def __init__(self, kernelSize=7):
        super().__init__()
        self.conv2d = nn.Conv2d(2, 1, kernel_size=kernelSize, padding="same")
    ...
    def forward(self, x):
        ...
        pooled = torch.cat([maxPool, avgPool], dim=1)

        weights = self.conv2d(pooled)
...
class TripletAttention(nn.Module):
    def __init__(self, kernelSize=7):
        super().__init__()
        self.branch1 = ZPoolAttention(kernelSize)
        self.branch2 = ZPoolAttention(kernelSize)
        self.branch3 = ZPoolAttention(kernelSize)

    def forward(self, x):
        x1 = x.permute(0, 3, 2, 1)
        x1 = x1 * self.branch1(x1)
        x1 = x1.permute(0, 3, 2, 1)

        x2 = x.permute(0, 2, 1, 3)
        x2 = x2 * self.branch2(x2)
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc:
        ...
        torch.save(model.state_dict(), 'best_models/TripletAttNet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/TripletAttNet.pt'))
model.eval()
testCorrect = 0
testTotal = 0

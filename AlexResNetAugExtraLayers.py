...
class EmotionResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        identity = self.bn_proj3(self.proj3(out))
        out = self.relu(self.bn3a(self.conv3a(out)))
        out = self.bn3b(self.conv3b(out))
        out = self.relu(out + identity)

        identity = self.bn_proj4(self.proj4(out))
        out = self.relu(self.bn4a(self.conv4a(out)))

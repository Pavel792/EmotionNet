...
class EmotionCNN(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.se1 = CSABlock(64, reduction=8)
        self.se2 = CSABlock(128, reduction=8)
        self.se3 = CSABlock(256)
        self.se4 = CSABlock(512)
        self.se5 = CSABlock(512)
...
class Squeeze(nn.Module):
    ...
    def forward(self, x):
        ...
        pooled = self.avgPool(x).view(batch, channels)

        newX = x.view(batch, channels, H * W)

        dist = torch.cdist(newX, newX, p=2)

        meanDist = dist.mean(dim=(1, 2), keepdim=True)

        v = torch.exp(-dist / meanDist)

        mask = 1 - torch.eye(channels, device=x.device)

        v = v * mask

        summ = v.sum(dim=(1, 2), keepdim=True)

        w = v / summ

        mean = pooled.mean(dim=1, keepdim=True)
        std = pooled.std(dim=1, keepdim=True) 
        z = (pooled - mean) / (std + 1e-6)  

        zCol = z.unsqueeze(-1)
        zRow = z.unsqueeze(1)

        outerProduct =torch.bmm(zCol, zRow)

        weighted = torch.bmm(outerProduct, w)
        moranI = torch.diagonal(weighted, dim1=1, dim2=2)

        moranMean = moranI.mean(dim=1, keepdim=True)
        moranStd = moranI.std(dim=1, keepdim=True)

        q = (moranI - moranMean) / (moranStd + 1e-6)
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc or _e == 0:
        ...
        torch.save(model.state_dict(), 'best_models/CSANet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/CSANet.pt'))
model.eval()
testCorrect = 0
testTotal = 0

...
class atentionBetweenChannels(nn.Module):
    ...
    def forward(self, x):
        ...
        max = self.maxPool(x).view(batch, channels)
        max = self.mlp(max)
        
        weights = self.sigmoid(avg + max).view(batch, channels, 1, 1)
...
class depthWise(nn.Module):
    def __init__(self, channels, kernelSize):
        super().__init__()
        self.conv = nn.Conv2d(channels, channels, kernel_size=kernelSize, padding="same", groups=channels)

    def forward(self, x):
        return self.conv(x)

class stringDepthWise(nn.Module):
    def __init__(self, channels, kernelSize):
        super().__init__()
        self.convH = nn.Conv2d(channels, channels, kernel_size=(1, kernelSize), padding="same", groups=channels)
        self.convV = nn.Conv2d(channels, channels, kernel_size=(kernelSize, 1), padding="same", groups=channels)
...
class attentionInOneChannel(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.channels = channels
        self.dwconv = depthWise(channels, kernelSize=5)
        self.branch0 = nn.Identity()
        self.branch1 = stringDepthWise(channels, 7)
        self.branch2 = stringDepthWise(channels, 11)
        self.branch3 = stringDepthWise(channels, 21)
        self.mixConv = nn.Conv2d(channels, channels, kernel_size=1)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        weights = self.sigmoid(self.mixConv(summed))
...
class CPCA(nn.Module):
    def __init__(self, channels, reduction=8):
        super().__init__()
        self.channel_attention = atentionBetweenChannels(channels, reduction)
        self.spatial_attention = attentionInOneChannel(channels)
...
model = EmotionAttentionNet(numClasses=7).to(device)
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc or _e == 0:
        ...
        torch.save(model.state_dict(), 'best_models/CPCA.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/CPCA.pt'))
model.eval()
testCorrect = 0
testTotal = 0

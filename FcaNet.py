...
class EmotionCNN(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.fca1 = FcaBlock(64, H=100, W=100, reduction=8)
        self.fca2 = FcaBlock(128, H=50, W=50, reduction=8)
        self.fca3 = FcaBlock(256, H=25, W=25)
        self.fca4 = FcaBlock(512, H=12, W=12)
        self.fca5 = FcaBlock(512, H=6, W=6)
...
def getDctFilter(H, W, u, v):
    filt = torch.zeros(H, W)
    for i in range(H):
        for j in range(W):
            filt[i, j] = torch.cos(torch.tensor(torch.pi * u / H * (i + 0.5))) * torch.cos(torch.tensor(torch.pi * v / W * (j + 0.5)))
    return filt

class MultiSpectralDCTLayer(nn.Module):
    def __init__(self, channels, H, W, freqIndices):
        super().__init__()
        self.numFreq = len(freqIndices)
        assert channels % self.numFreq == 0

        filters = []
        for (u, v) in freqIndices:
            filters.append(getDctFilter(H, W, u, v))
        self.register_buffer('dctFilters', torch.stack(filters))

    def forward(self, x):
        batch, channels, H, W = x.shape
        groupSize = channels // self.numFreq

        parts = torch.split(x, groupSize, dim=1)

        results = []
        for i, part in enumerate(parts):
            filt = self.dctFilters[i]
            weighted = part * filt
            freq = weighted.sum(dim=(2, 3))
            results.append(freq)
...
class FcaBlock(nn.Module):
    def __init__(self, channels, H, W, reduction=16, freqIndices=[(0, 0), (0, 1), (1, 0), (1, 1)]):
        super().__init__()

        self.sq = MultiSpectralDCTLayer(channels, H, W, freqIndices)
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc:
        ...
        torch.save(model.state_dict(), 'best_models/FcaNet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/FcaNet.pt'))
model.eval()
testCorrect = 0
testTotal = 0

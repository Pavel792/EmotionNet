...
class SimAM(nn.Module):
    ...
    def forward(self, x):
        ...
        sigma2 = var + self.e_lambda
        energy = (x - mu) ** 2 / (2 * sigma2) + 2 * sigma2
        
        attention_weights = torch.sigmoid(1 / (energy + 1e-8))
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc:
        ...
        torch.save(model.state_dict(), 'best_models/SimAMNet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/SimAMNet.pt'))
model.eval()
testCorrect = 0
testTotal = 0

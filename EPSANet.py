...
class EmotionCNN(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.att1 = PSAModule(64)
        self.att2 = PSAModule(128)
        self.att3 = PSAModule(256)
        self.att4 = PSAModule(512)
        self.att5 = PSAModule(512)
...
class Squeeze(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgPool(x).view(batch, channels)
...
class SEBlock(nn.Module):
    def __init__(self, channels, reduction=8):
        ...
        self.sq = Squeeze(channels)
        self.ex = Excitation(channels, reduction)

    def forward(self, x):
        batch, channels, _, _ = x.shape

        weights = self.sq(x)
        weights = self.ex(weights)
        weights = weights.view(batch, channels, 1, 1)

        return x * weights

class PSAModule(nn.Module):
    def __init__(self, channels, groupNumber=4):
        super().__init__()
        self.channels = channels
        self.groupNumber = groupNumber
        self.relu = nn.ReLU()

        self.conv0 = nn.Conv2d(self.channels, self.channels // self.groupNumber, kernel_size=3, padding="same", groups=2)
        self.conv1 = nn.Conv2d(self.channels, self.channels // self.groupNumber, kernel_size=5, padding="same", groups=4)
        self.conv2 = nn.Conv2d(self.channels, self.channels // self.groupNumber, kernel_size=7, padding="same", groups=8)
        self.conv3 = nn.Conv2d(self.channels, self.channels // self.groupNumber, kernel_size=9, padding="same", groups=16)

        self.sq0 = Squeeze(self.channels // self.groupNumber)
        self.sq1 = Squeeze(self.channels // self.groupNumber)
        self.sq2 = Squeeze(self.channels // self.groupNumber)
        self.sq3 = Squeeze(self.channels // self.groupNumber)

        self.ex0 = Excitation(self.channels // self.groupNumber)
        self.ex1 = Excitation(self.channels // self.groupNumber)
        self.ex2 = Excitation(self.channels // self.groupNumber)
        self.ex3 = Excitation(self.channels // self.groupNumber)
    ...
    def forward(self, x):
        ...
        z0 = self.ex0(self.sq0(f0))
        z1 = self.ex1(self.sq1(f1))
        z2 = self.ex2(self.sq2(f2))
        z3 = self.ex3(self.sq3(f3))

        stacked = torch.stack([z0, z1, z2, z3], dim=1)
        att = torch.softmax(stacked, dim=1)

        att0, att1, att2, att3 = att[:, 0], att[:, 1], att[:, 2], att[:, 3]

        y0 = f0 * att0.view(f0.shape[0], -1, 1, 1)
        y1 = f1 * att1.view(f1.shape[0], -1, 1, 1)
        y2 = f2 * att2.view(f2.shape[0], -1, 1, 1)
        y3 = f3 * att3.view(f3.shape[0], -1, 1, 1)
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc or _e == 0:
        ...
        torch.save(model.state_dict(), 'best_models/EPSANet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/EPSANet.pt'))
model.eval()
testCorrect = 0
testTotal = 0
...
# Max Acc without overfitting 0.7448

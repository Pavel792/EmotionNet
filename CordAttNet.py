...
class EmotionCNN(nn.Module):
    def __init__(self, numClasses=7):
        ...
        self.ca5 = CoordinateAttention(512)
...
class CoordinateAttention(nn.Module):
    def __init__(self, channels, reduction=16):
        ...
        self.convH = nn.Conv2d(mid, channels, kernel_size=1)
        self.convW = nn.Conv2d(mid, channels, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        batch, channels, H, W = x.shape

        xH = self.poolH(x)                      # [batch, channels, H, 1]
        xW = self.poolW(x).permute(0, 1, 3, 2)   # [batch, channels, W, 1]

        y = torch.cat([xH, xW], dim=2)           # [batch, channels, H+W, 1]
        y = self.act(self.bn1(self.conv1(y)))    # [batch, mid, H+W, 1]

        yH, yW = torch.split(y, [H, W], dim=2)
        yW = yW.permute(0, 1, 3, 2)              # [batch, mid, 1, W]

        weightsH = self.sigmoid(self.convH(yH))  # [batch, channels, H, 1]
        weightsW = self.sigmoid(self.convW(yW))  # [batch, channels, 1, W]

        return x * weightsH * weightsW
...
for _e in range(epochs):
    ...
    if valAccuracy > bestValAcc:
        ...
        torch.save(model.state_dict(), 'best_models/CordAttNet.pt')
        print(f'Сохранена лучшая модель: Val Acc {bestValAcc:.4f}')

    scheduler.step(avgValLoss)


model.load_state_dict(torch.load('best_models/CordAttNet.pt'))
model.eval()
testCorrect = 0
testTotal = 0
